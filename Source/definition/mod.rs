		...
		S: serde::Serializer,
	{
		...
		state.serialize_field(
			...
			match self.key {
				...
				ARIAProperty::ARIAState(ARIAState::AriaChecked) => {
					"aria-checked"
				},
				ARIAProperty::ARIAState(ARIAState::AriaDisabled) => {
					"aria-disabled"
				},
				ARIAProperty::ARIAState(ARIAState::AriaExpanded) => {
					"aria-expanded"
				},
				ARIAProperty::ARIAState(ARIAState::AriaGrabbed) => {
					"aria-grabbed"
				},
				ARIAProperty::ARIAState(ARIAState::AriaHidden) => "aria-hidden",
				ARIAProperty::ARIAState(ARIAState::AriaInvalid) => {
					"aria-invalid"
				},
				ARIAProperty::ARIAState(ARIAState::AriaPressed) => {
					"aria-pressed"
				},
				ARIAProperty::ARIAState(ARIAState::AriaSelected) => {
					"aria-selected"
				},
			},
		)?;
...
pub struct ARIARoleRelationConceptConstraints {
	...
	pub direct_descendant_of_details_element_with_the_open_attribute_defined:
		bool,
...
pub type ARIAProps =
	OrderedMap<&'static str, &'static Option<ARIAPropertyCurrent>>;
...
pub struct ARIARoleDefinition {
	...
	pub required_owned_elements:
		Option<&'static [&'static [&'static ARIARole]]>,
	...
	pub super_class:
		Option<&'static [&'static [&'static ARIARoleDefinitionKey]]>,
}
...
pub type RoleDefinitions =
	OrderedMap<&'static str, &'static ARIARoleDefinition>;
